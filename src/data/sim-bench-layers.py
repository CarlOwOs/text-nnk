...
if __name__ == "__main__":
    ...
    resultsABSum, resultsABAvg, resultsMinS, resultsAvgMinS, resultsAvg = {}, {}, {}, {}, {}
    for i in tqdm(range(13), total=13):
        predABSum, predABAvg, predMinS, predAvgMinS, predAvg = [], [], [], [], []
        for j in tqdm(range(len(dataset)), total=len(dataset)):
            embed1, embed2 = features1[j][i][:,1:-1, :].squeeze(0), features2[j][i][:,1:-1, :].squeeze(0) # Lx768
        ...
        resultsABSum[f"Layer-{i}"] = spearmanr(dataset["similarity_score"], predABSum)[0]*100
        resultsABAvg[f"Layer-{i}"] = spearmanr(dataset["similarity_score"], predABAvg)[0]*100
        resultsMinS[f"Layer-{i}"] = spearmanr(dataset["similarity_score"], predMinS)[0]*100
        resultsAvgMinS[f"Layer-{i}"] = spearmanr(dataset["similarity_score"], predAvgMinS)[0]*100
        resultsAvg[f"Layer-{i}"] = spearmanr(dataset["similarity_score"], predAvg)[0]*100
    
    results = pd.DataFrame({"ABSum": resultsABSum, "ABAvg": resultsABAvg, "MinS": resultsMinS, "AvgMinS": resultsAvgMinS, "Avg": resultsAvg})
    results.to_csv("data/results-layers.csv")