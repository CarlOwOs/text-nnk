...
if __name__ == "__main__":
    ...
    dataset_embeddings = [model(**tokenized_sentence.to(device))[0].mean(axis=1) for tokenized_sentence in dataset_tokens]
    features = torch.cat(dataset_embeddings, dim=0)

    weight_values, error = nnk_graph(features, top_k=5, cuda_available=cuda_available, device=device)
